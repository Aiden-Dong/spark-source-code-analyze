...
private[spark] class DAGScheduler(
  ...
  private[scheduler] val stageIdToStage = new HashMap[Int, Stage]    // 已经解析完成的 Stage 的集合 stageId -> stage
  /**
  ...
  /**
   * 获取这个RDD的所有上游依赖 Shuffle Dependency
   * 基于 ShuffleDependency 构建依赖测的 Stage
   */
  ...
  private[scheduler] def getShuffleDependencies(
    ...
    val visited = new HashSet[RDD[_]]                         // 用于记录这个RDD是否被检查过
    val waitingForVisit = new ArrayStack[RDD[_]]

    waitingForVisit.push(rdd)   // root rdd 入栈
    ...
    while (waitingForVisit.nonEmpty) {
      ...
      if (!visited(toVisit)) {
        visited += toVisit
        toVisit.dependencies.foreach {   // 遍历所有的依赖
          case shuffleDep: ShuffleDependency[_, _, _] =>
            parents += shuffleDep                      // 记录遇到的ShuffleDependency, 并停止继续切割
          case dependency =>
            waitingForVisit.push(dependency.rdd)       // 如果是遇到非shuffleDependency, 就继续往下遍历
        }
      }
    }
    parents
  ...
  def submitJob[T, U](
    ...
    // 通过 JobSubmitted 事件， 将 rdd 提交给 eventProcessLoop 去执行
  ...
  /****
   * DAGSchedulerEventProcessLoop  回调处理的 JobSubmit 事件
   * @param jobId
   * @param finalRDD
   * @param func
   * @param partitions
