...
class HadoopRDD[K, V](
    ...
    inputFormatClass: Class[_ <: InputFormat[K, V]],     // 用于读取文件的InputFormat
    keyClass: Class[K],
    valueClass: Class[V],
    minPartitions: Int)
  ...
  protected def getJobConf(): JobConf = {
    val conf: Configuration = broadcastedConf.value.value  // 广播的SparkConfig
  ...
  override def compute(theSplit: Partition, context: TaskContext): InterruptibleIterator[(K, V)] = {
    ...
    val iter = new NextIterator[(K, V)] {
      ...
      private val inputMetrics = context.taskMetrics().inputMetrics    // 获取输入计数器
      ...
      override def getNext(): (K, V) = {
        ...
        if (!finished) {
          inputMetrics.incRecordsRead(1)  // 迭代读取记录数
        }
