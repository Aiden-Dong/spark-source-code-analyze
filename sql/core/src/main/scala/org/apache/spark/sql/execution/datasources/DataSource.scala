...
case class DataSource(
    ...
    paths: Seq[String] = Nil,         // paths
    userSpecifiedSchema: Option[StructType] = None,  // 用户指定 schema
    partitionColumns: Seq[String] = Seq.empty,
    bucketSpec: Option[BucketSpec] = None,
    options: Map[String, String] = Map.empty,       // 读取数据参数
    catalogTable: Option[CatalogTable] = None) extends Logging {

  case class SourceInfo(name: String, schema: StructType, partitionColumns: Seq[String])

  // 基于提供的 format 信息， 拿到对应的DataSource Class
  lazy val providingClass: Class[_] = DataSource.lookupDataSource(className, sparkSession.sessionState.conf)
  ...
  def resolveRelation(checkFilesExist: Boolean = true): BaseRelation = {
    val relation = (providingClass.newInstance(), userSpecifiedSchema) match {
      ...
      // 我们正在从流式查询的结果中读取数据。
      // 为了加载文件，我们将使用元数据日志而不是使用 HDFS API 来列出这些文件。
      ...
      case (format: FileFormat, _) =>
        ...
        val (fileCatalog, dataSchema, partitionSchema) = if (useCatalogFileIndex) {
          ...
          val (resultDataSchema, resultPartitionSchema) =
            getOrInferFileFormatSchema(format, Some(index))  // 获取文件 schema
...
object DataSource extends Logging {
  ...
  /****
   * 基于用户配置的 format， 查找对应的DataSource Class
   * @param provider 用户指定的 ReadFormat
   * @param conf    SQL Config
   * @return Class Of Format
   */
  def lookupDataSource(provider: String, conf: SQLConf): Class[_] = {
    ...
    // 获取Spark ClassLoader,名称必须为 DefaultSource
    val loader = Utils.getContextOrSparkClassLoader

    // 加载 DataSourceRegister 的实现类
    val serviceLoader = ServiceLoader.load(classOf[DataSourceRegister], loader)

    try {
      // 遍历 DataSourceRegister 的实现类
      serviceLoader.asScala.filter(_.shortName().equalsIgnoreCase(provider1)).toList match {
        ...
        case head :: Nil =>
          ...
          head.getClass
        case sources =>
          // 输入有多个已注册的别名。如果有一个数据源的前缀包含"org.apache.spark"包，我们会使用它，
          // 因为它被认为是Spark内部的数据源。
