...
class DataFrameReader private[sql](sparkSession: SparkSession) extends Logging {
  ...
  /**
   * 将输入加载为DataFrame，适用于支持多路径的数据源。
   * 仅当数据源是HadoopFsRelationProvider时才有效。
   *
   * @since 1.6.0
   */
  @scala.annotation.varargs
  def load(paths: String*): DataFrame = {
    // hive format 不能使用 load
  ...
  private def loadV1Source(paths: String*) = {
    ...
    sparkSession.baseRelationToDataFrame(
      DataSource.apply(
        ...
        paths = paths, // 文件读取路径
        userSpecifiedSchema = userSpecifiedSchema,  // 用户指定 schema
        className = source,
  ...
  ///////////////////////////////////////////////////////////////////////////////////////
  // format : orc / csv / parquet / json / txt 等信息
